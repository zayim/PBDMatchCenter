...
$klupa_ime = array(); $klupa_broj = array();
$broj_igraca_na_klupi=mysql_num_rows($rez)-11;
for ($i=0; $i<$broj_igraca_na_klupi; $i++)
{
	$red=mysql_fetch_assoc($rez);
	$klupa_ime[]=$red['ime']; 
	$klupa_broj[]=$red['broj_na_dresu'];
}
...
Powered by: <a href="http://www.barcabih.com" style="cursor:url(../slike/cursor.png), pointer;"> Bar&#231;a BiH</a> | Zvanični sastav
</span>
</center>
...
<?php
	...
	$broj_redova_na_klupi=($broj_igraca_na_klupi)/4;
	for ($i=0; $i<$broj_redova_na_klupi; $i++)
	{
		$klupa1_ime=""; $klupa1_broj=""; $klupa2_ime=""; $klupa2_broj=""; $klupa3_ime=""; $klupa4_broj=""; $klupa4_ime=""; $klupa4_broj="";
		if ($i*4+1 <= $broj_igraca_na_klupi) { $klupa1_broj=$klupa_broj[$i*4]; $klupa1_ime=$klupa_ime[$i*4]; }
		if ($i*4+2 <= $broj_igraca_na_klupi) { $klupa2_broj=$klupa_broj[$i*4+1]; $klupa2_ime=$klupa_ime[$i*4+1]; }
		if ($i*4+3 <= $broj_igraca_na_klupi) { $klupa3_broj=$klupa_broj[$i*4+2]; $klupa3_ime=$klupa_ime[$i*4+2]; }
		if ($i*4+4 <= $broj_igraca_na_klupi) { $klupa4_broj=$klupa_broj[$i*4+3]; $klupa4_ime=$klupa_ime[$i*4+3]; }
		
		echo "<center><table><tr>";
		
		if ($klupa1_broj != "" && $klupa1_ime!="")
		{
			echo <<< _END
			<td class="igrac_klupa">
    		<span style="color:#0CF"> $klupa1_broj  </span>
         	$klupa1_ime
    		</td>
_END;
		}
		
		if ($klupa2_broj != "" && $klupa2_ime!="")
		{
			echo <<< _END
			<td class="igrac_klupa">
    		<span style="color:#0CF"> $klupa2_broj  </span>
         	$klupa2_ime
    		</td>
_END;
		}
		
		if ($klupa3_broj != "" && $klupa3_ime!="")
		{
			echo <<< _END
			<td class="igrac_klupa">
    		<span style="color:#0CF"> $klupa3_broj  </span>
         	$klupa3_ime
    		</td>
_END;
		}
		
		if ($klupa4_broj != "" && $klupa4_ime!="")
		{
			echo <<< _END
			<td class="igrac_klupa">
    		<span style="color:#0CF"> $klupa4_broj  </span>
         	$klupa4_ime
    		</td>
_END;
		}
